...
function solution(park, routes) {
    ...
    park = park.map((str, y) => {
        const x = str.indexOf('S');
        
        if(x > -1) {
            current[0] = y;
            current[1] = parseInt(x);
        }
    ...
    for(let route of routes) {
        ...
        const moveCnt = parseInt(route[1]);
        ...
        if(direction === 'E' || direction === 'W') {
            ...
            if(direction === 'E') {
                if(currentX + moveCnt > park[currentY].length - 1) {
                    continue;
                }
            } else {
                if(currentX - moveCnt < 0) {
                    continue;
                }
            }

            for(let i = 1; i <= moveCnt; i++) {
                if(park[currentY][currentX + (direction === 'E' ? i : -i)] === 'X') {
                    move = false;
                    break;
                }
            }
            ...
            if(move) {
                current[1] = currentX + (direction === 'E' ? moveCnt : -moveCnt);
            }
            
        } else {
            
            if(direction === 'S') {
                if(currentY + moveCnt > park.length - 1) {
                    continue;
                }
            } else {
                if(currentY - moveCnt < 0) {
                    continue;
                }
            }

            for(let i = 1; i <= moveCnt; i++) {
                if(park[currentY + (direction === 'S' ? i : -i)][currentX] === 'X') {
                    move = false;
                    break;
                }
            ...
            if(move) {
                current[0] = currentY + (direction === 'S' ? moveCnt : -moveCnt);
            }
        }
    }
