...
function solution(babbling) {
    ...
    for(let babble of babbling) {
        let prev_word = '';
        for(let i = 0; i < babble.length; i=0) {
            let char = babble[i];
            let current_word = '';
            
            if(char === 'a' || char === 'w') {
                i = 3;
            } else if(char === 'y' ||  char === 'm') {
                i = 2;
            } else {
                break;
            }
            
            current_word = babble.substr(0, i);
            
            if(prev_word === babble.substr(0, i)) {
                break;
            }
            
            prev_word = current_word;
            
            if(current_word === words[char]) {
                babble = babble.substr(i);
            } else {
                break;
            }
...
//번외로 내가 풀이한 코드가 초라해지는 정규표현식을 사용한 풀이법이다.
//정규표현식도 공부하자..
function solution(babbling) {
